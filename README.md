...
- **「開く」リンクの遷移先**: リポジトリに `homepage` が設定されていればそれを優先し、なければ GitHub Pages が有効なリポジトリはそのURL（`https://<owner>.github.io/<repo>/`）を自動で使用します。アプリのURLとGitHubリポジトリの両方が分かる場合は、カード右下が「開く ▾」のプルダウンになり、「アプリを開く」「GitHubで見る」を選んで新しいタブで開けます（自動判定したURLが違う場合も、GitHub側は必ず選べます）。
