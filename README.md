...
- **「アプリを選択」プルダウン**: GitHub同期したカードにカーソルを合わせる（またはフォーカスする）と、そのリポジトリ内のHTMLファイル一覧を自動取得してプルダウンに表示します。GitHub Pagesが有効なら各HTMLファイルの公開URLへ、無効な場合は [htmlpreview.github.io](https://htmlpreview.github.io/) 経由でプレビューできるURLへ変換します。`homepage` が設定されていればそれも選択肢に加わり、最後に必ず「GitHubで見る」も選べます。
